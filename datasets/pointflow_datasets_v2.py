# v2
# use 90:10 split for whole dataset
import os
import random
...
class Uniform15KPC(Dataset):
    def __init__(self, root_dir, subdirs, tr_sample_size=10000,
        ...
        if not fps:
            ...
            for cate_idx, subd in enumerate(self.subdirs):
                # NOTE: [subd] here is synset id
                sub_path = os.path.join(root_dir, subd, "all")
                if not os.path.isdir(sub_path):
                    print("Directory missing : %s" % sub_path)
                    continue

                h5_file = os.path.join(sub_path, "consolidated_%d.h5" % tr_sample_size)
                if os.path.isfile(h5_file):
                    with h5py.File(h5_file, "r") as f:
                        a_group_key = list(f.keys())[0]
                        # Get the data
                        point_cloud = list(f[a_group_key])

                    self.shuffle_idx = list(range(len(point_cloud)))
                    random.Random(38383).shuffle(self.shuffle_idx)
                    point_cloud = [point_cloud[i] for i in self.shuffle_idx]
                    point_cloud = point_cloud[:int(len(point_cloud) * 0.85)] if self.split == "train" else point_cloud[int(len(point_cloud) * 0.85):]
                    self.all_points.append(point_cloud)
                else:
                    print(f"{h5_file} does not exist")
        ...
        self.train_points = self.all_points[:, :10000] if not self.fps else self.all_points
        self.test_points = self.all_points[:, 10000:] if not self.fps else self.all_points
    ...
    def __getitem__(self, idx):
        ...
        cate_idx = self.cate_idx_lst[idx] if not self.fps else  ""
        sid, mid = self.all_cate_mids[idx] if not self.fps else "", ""
...
class ShapeNet15kPointClouds(Uniform15KPC):
    def __init__(
        ...
        self.root_dir = root_dir
        self.split = split
        assert self.split in ['all', 'train', 'test', 'val']
