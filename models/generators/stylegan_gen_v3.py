...
from trainers.utils.utils import (ForkedPdb, get_edge_features, knn,
                                  next_power_of_2, normalize_2nd_moment)
...
class SynthesisNetwork(nn.Module):
    def __init__(
        ...
        # [256, 256, 256, 128, 64, 32, 16, 8]
        channels_dict = {res: min(channel_base // res, channel_max) for res in self.blk_res}

        self.num_ws = 0
        for res in self.blk_res:
            # increase neighbor
            # self.num_k = res // 4 if res <= 64 else num_k
            self.num_k = res // 4 if res <= 128 else num_k
...
class SynthesisBlock(nn.Module):
    def __init__(
        ...
        if self.in_channels == 0:
            ...
            if fuseconv:
                ...
                self.conv1 = EdgeConv_Global(in_channels, out_channels, num_pts, num_k)
            self.num_conv += 1
        ...
        self.conv2 = EdgeConv_Global(out_channels, out_channels, num_pts, num_k)
        self.num_conv += 1
    ...
    def forward(self, content, pc, style):
        ...
        if self.is_last or self.architecture == 'skip':
            # pc = self.pc_up(pc) if pc is not None else None
            pc = F.interpolate(pc, scale_factor=2) if pc is not None else None
...
class EdgeConv_Global(nn.Module):
    def __init__(self, in_channels, out_channels, num_pts, num_k=20):
        super(EdgeConv_Global, self).__init__()
        self.num_pts = num_pts
        self.maxpool = nn.MaxPool2d((1, num_pts), (1, 1))
        
        self.ec = EdgeConv(in_channels, in_channels, num_k)
        ...
        self.fc_out = nn.Sequential(
            nn.Conv1d(2 * in_channels, out_channels, 1),
    ...
    def forward(self, x, pc=None):
        ...
        # global feature path
        x_g = self.maxpool(x)
        x_g = x_g.view(batchsize, -1)
        x_g = self.fc(x_g)
        x_g = x_g.view(batchsize, -1, 1)
        x_g = x_g.repeat(1, 1, num_pts)
        
        # edge convolution path
        x_l = self.ec(x)
        x_all = torch.cat((x_g, x_l), 1)

        x_out = self.fc_out(x_all)
        return x_out
...
class ToPCLayer(nn.Module):
    def __init__(self, in_channels, w_dim, pc_dim=3, clamp=1):
        ...
        self.hidden = 64
        self.fc = nn.Sequential(
            nn.Conv1d(in_channels, self.hidden, 1),
            nn.LeakyReLU(0.2),
            nn.Conv1d(self.hidden, self.hidden // 4 , 1),
            nn.LeakyReLU(0.2),
            nn.Conv1d(self.hidden // 4, pc_dim, 1),
            nn.LeakyReLU(0.2)
        )
...
class Upsample(nn.Module):
    ...
    def forward(self, x, pc=None):
        ...
        # use stack instead of cat to preserve position information
        # interleave two tensors
        # ForkedPdb().set_trace()
        x_up = torch.stack([x, neighbor + x], dim=2).view(B, -1, dims)
