...
from trainers.utils.utils import (ForkedPdb, get_edge_features, knn, kfn,
...
class Upsample(nn.Module):
    ...
    def forward(self, x, pc=None):
        ...
        # use dist threshold
        if pc is not None:
            dist, idx = kfn(pc, self.num_k + 1)
        else:
            dist, idx = kfn(x, self.num_k + 1)
        idx = idx[:, :, 1:]                                 # [B, N, k]
        dist = dist[:, :, 1:]                               # [B, N, k]
        ...
        neighbor = edge.sum(dim=3) / (self.num_k + 1)
