...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        self.num_points      = cfgmodel.num_points
        self.bottleneck_size = cfgmodel.bottleneck_size
        self.n_primitives    = cfgmodel.n_primitives
        self.use_AdaIn       = cfgmodel.use_AdaIn
        self.hide_size       = cfgmodel.hide_size
        self.use_SElayer     = cfgmodel.use_SElayer
    ...
    def forward(self, style, gt):

        # prepare proper data
        gt = gt.transpose(1, 2).contiguous()  # [bs, 3, in_points]
        assert style.size(0) == gt.size(0)
        # decode
        outs = self.decoder(style)
        ...
        # refine first time
        # middle, loss_mst = self.refine(outs, gt, coarse)

        # refine second time
        # outs_2 = middle.transpose(1, 2).contiguous()
        # refine, _ = self.refine(outs_2, gt, middle)
        ...
        if self.use_AdaIn == "share":
            ...
            for i in range(self.n_primitives):
                ...
                regular_grid = regular_grid.expand(
                    bs, regular_grid.size(1), regular_grid.size(2)
                ).contiguous()
